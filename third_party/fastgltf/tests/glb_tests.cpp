...
TEST_CASE("Load basic GLB file", "[gltf-loader]") {
  ...
  SECTION("Load basic Box.glb") {
    auto asset =
        parser.loadBinaryGLTF(&jsonData, folder, fastgltf::Options::None,
  ...
  SECTION("Load basic Box.glb and load buffers") {
    auto asset = parser.loadBinaryGLTF(&jsonData, folder,
    ...
    REQUIRE(static_cast<uint64_t>(bufferVector->bytes.size() -
                                  buffer.byteLength) < 3);
  }
  ...
  SECTION("Load GLB by bytes") {
    ...
    REQUIRE(byteBuffer.fromByteView(bytes.data(), length,
                                    length + fastgltf::getGltfBufferPadding()));

    auto asset = parser.loadBinaryGLTF(&byteBuffer, folder,
                                       fastgltf::Options::LoadGLBBuffers,
                                       fastgltf::Category::Buffers);
    REQUIRE(asset.error() == fastgltf::Error::None);
  }
