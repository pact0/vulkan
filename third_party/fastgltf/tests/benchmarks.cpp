...
constexpr auto benchmarkOptions =
...
std::string tinygltf_ExpandFilePathFunction(const std::string &path,
                                            [[maybe_unused]] void *user) {
  return path;
}

bool tinygltf_ReadWholeFileFunction(std::vector<unsigned char> *data,
                                    std::string *, const std::string &,
                                    void *) {
  // tinygltf checks if size == 1. It also checks if the size is correct for glb
  // files, but well ignore that for now.
  data->resize(1);
  return true;
}

bool tinygltf_LoadImageData(tinygltf::Image *image, const int image_idx,
...
TEST_CASE("Benchmark loading of NewSponza", "[gltf-benchmark]") {
  if (!std::filesystem::exists(intelSponza / "NewSponza_Main_glTF_002.gltf")) {
    // NewSponza is not part of gltf-Sample-Models, and therefore not always
    // available.
  ...
  REQUIRE(jsonData->fromByteView(
      bytes.data(), bytes.size() - fastgltf::getGltfBufferPadding(),
      bytes.size()));
  ...
  BENCHMARK("Parse NewSponza with gltf-rs") {
    auto slice = rust::Slice<const std::uint8_t>(
        reinterpret_cast<std::uint8_t *>(bytes.data()), bytes.size() - padding);
  ...
  BENCHMARK("Parse NewSponza with assimp") {
    return aiImportFileFromMemory(reinterpret_cast<const char *>(bytes.data()),
                                  jsonData->getBufferSize(), 0, nullptr);
  };
#endif
}
...
TEST_CASE("Benchmark base64 decoding from glTF file", "[gltf-benchmark]") {
  ...
  auto cylinderEngine =
      sampleModels / "2.0" / "2CylinderEngine" / "glTF-Embedded";
  ...
  BENCHMARK("2CylinderEngine decode with tinygltf") {
    return tinygltf.LoadASCIIFromString(&model, &err, &warn,
                                        reinterpret_cast<char *>(bytes.data()),
                                        bytes.size(), cylinderEngine.string());
  };
#endif
  ...
  BENCHMARK("2CylinderEngine with assimp") {
    const auto *scene =
        aiImportFileFromMemory(reinterpret_cast<const char *>(bytes.data()),
                               jsonData->getBufferSize(), 0, nullptr);
...
TEST_CASE("Benchmark massive gltf file", "[gltf-benchmark]") {
  if (!std::filesystem::exists(bistroPath / "bistro.gltf")) {
    // Bistro is not part of gltf-Sample-Models, and therefore not always
    // available.
...
TEST_CASE("Compare parsing performance with minified documents",
          "[gltf-benchmark]") {
  auto buggyPath = sampleModels / "2.0" / "Buggy" / "glTF";
  auto bytes = readFileAsBytes(buggyPath / "Buggy.gltf");
  auto jsonData = std::make_unique<fastgltf::GltfDataBuffer>();
  ...
  REQUIRE(minifiedJsonData->fromByteView(
      minified.data(), minified.size() - fastgltf::getGltfBufferPadding(),
      minified.size()));
...
TEST_CASE("Small CRC32-C benchmark", "[gltf-benchmark]") {
  ...
  BENCHMARK("Default 1-byte tabular algorithm") {
    return fastgltf::crc32c(reinterpret_cast<const std::uint8_t *>(test.data()),
                            test.size());
  };
  BENCHMARK("SSE4 hardware algorithm") {
    return fastgltf::hwcrc32c(
        reinterpret_cast<const std::uint8_t *>(test.data()), test.size());
  };
}
#endif
...
TEST_CASE("Compare base64 decoding performance", "[gltf-benchmark]") {
  ...
  BENCHMARK("Run cgltf's base64 decoder") {
    ...
    auto outputSize =
        fastgltf::base64::getOutputSize(generatedData.size(), padding);
    std::string output;
    output.resize(outputSize);
    auto *outputData = output.data();
    return cgltf_load_buffer_base64(&options, generatedData.size(),
                                    generatedData.data(),
  ...
  BENCHMARK("Run base64 Rust library decoder") {
    auto slice = rust::Slice<const std::uint8_t>(
        reinterpret_cast<std::uint8_t *>(generatedData.data()),
        generatedData.size());
  ...
  if (const auto *neon = impls["arm64"];
      avx2 != nullptr && neon->supported_by_runtime_system()) {
