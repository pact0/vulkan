...
constexpr std::string_view testBase64 =
    "SGVsbG8gV29ybGQuIEhlbGxvIFdvcmxkLiBIZWxsbyBXb3JsZC4=";
...
TEST_CASE("Check big base64 data decoding", "[base64]") {
  ...
  std::ifstream output(path / "base64.txt.out",
...
TEST_CASE("Test base64 buffer decoding", "[base64]") {
  ...
  SECTION("Validate large buffer load from glTF") {
    auto asset =
        parser.loadGLTF(tceJsonData.get(), cylinderEngine,
  ...
  SECTION("Validate base64 buffer and image load from glTF") {
    auto asset = parser.loadGLTF(
        btJsonData.get(), boxTextured, fastgltf::Options::None,
