...
TEST_CASE("Test all variants of CRC32-C hashing", "[gltf-loader]") {
  ...
  for (std::size_t i = 0; i < 256; ++i) {
    ...
    // We'll try and test if the hardware accelerated version generates the
...
TEST_CASE("Test extension stringification", "[gltf-loader]") {
  auto stringified =
      stringifyExtension(fastgltf::Extensions::EXT_meshopt_compression);
...
TEST_CASE("Test if glTF type detection works", "[gltf-loader]") {
  ...
  SECTION("glTF") {
    ...
    REQUIRE(fastgltf::determineGltfFileType(&jsonData) ==
            fastgltf::GltfType::glTF);
  ...
  SECTION("Invalid") {
    auto gltfPath = path / "base64.txt"; // Random file in the test directory
                                         // that's not a glTF file.
    REQUIRE(std::filesystem::exists(gltfPath));
    fastgltf::GltfDataBuffer jsonData;
    REQUIRE(jsonData.loadFromFile(gltfPath));
    REQUIRE(fastgltf::determineGltfFileType(&jsonData) ==
            fastgltf::GltfType::Invalid);
  }
}
...
TEST_CASE("Loading some basic glTF", "[gltf-loader]") {
  ...
  SECTION("Loading basic Cube.gltf") {
    ...
    auto cube = parser.loadGLTF(cubeJsonData.get(), cubePath, noOptions,
    ...
    REQUIRE(std::holds_alternative<fastgltf::Node::TRS>(
        cube->nodes.front().transform));

    REQUIRE(cube->accessors.size() == 5);
    REQUIRE(cube->accessors[0].type == fastgltf::AccessorType::Scalar);
    REQUIRE(cube->accessors[0].componentType ==
  ...
  SECTION("Loading basic Box.gltf") {
    ...
    auto box = parser.loadGLTF(boxJsonData.get(), boxPath, noOptions,
                               fastgltf::Category::OnlyRenderable);
...
TEST_CASE("Loading glTF animation", "[gltf-loader]") {
  ...
  fastgltf::Parser parser;
  auto asset = parser.loadGLTF(jsonData.get(), animatedCube, noOptions,
  ...
  REQUIRE(animation.samplers.front().interpolation ==
...
TEST_CASE("Loading glTF skins", "[gltf-loader]") {
  ...
  fastgltf::Parser parser;
  auto asset =
      parser.loadGLTF(jsonData.get(), simpleSkin, noOptions,
...
TEST_CASE("Loading glTF cameras", "[gltf-loader]") {
  ...
  fastgltf::Parser parser;
  auto asset = parser.loadGLTF(jsonData.get(), cameras, noOptions,
                               fastgltf::Category::Cameras);
  REQUIRE(asset.error() == fastgltf::Error::None);
  REQUIRE(fastgltf::validate(asset.get()) == fastgltf::Error::None);

  REQUIRE(asset->cameras.size() == 2);

  REQUIRE(std::holds_alternative<fastgltf::Camera::Perspective>(
      asset->cameras[0].camera));
  REQUIRE(std::holds_alternative<fastgltf::Camera::Orthographic>(
      asset->cameras[1].camera));
...
TEST_CASE("Test allocation callbacks for embedded buffers", "[gltf-loader]") {
  ...
  auto mapCallback = [](uint64_t bufferSize,
                        void *userPointer) -> fastgltf::BufferInfo {
  ...
  auto asset = parser.loadGLTF(jsonData.get(), boxPath, noOptions,
  ...
  const auto *customBuffer =
      std::get_if<fastgltf::sources::CustomBuffer>(&buffer.data);
...
TEST_CASE("Test base64 decoding callbacks", "[gltf-loader]") {
  ...
  auto decodeCallback = [](std::string_view encodedData, uint8_t *outputData,
                           size_t padding, size_t outputSize,
                           void *userPointer) {
  ...
  auto model = parser.loadGLTF(jsonData.get(), boxPath, noOptions,
                               fastgltf::Category::Buffers);
  REQUIRE(model.error() == fastgltf::Error::None);
  REQUIRE(fastgltf::validate(model.get()) == fastgltf::Error::None);
...
TEST_CASE("Test TRS parsing and optional decomposition", "[gltf-loader]") {
  SECTION("Test decomposition on glTF asset") {
    ...
    fastgltf::Parser parser;
    auto assetWithMatrix = parser.loadGLTF(jsonData.get(), path, noOptions,
    ...
    auto assetDecomposed = parser.loadGLTF(
        jsonData.get(), path, fastgltf::Options::DecomposeNodeMatrices,
    ...
    REQUIRE(std::holds_alternative<fastgltf::Node::TransformMatrix>(
        assetWithMatrix->nodes.back().transform));
    REQUIRE(std::holds_alternative<fastgltf::Node::TRS>(
        assetDecomposed->nodes.back().transform));

    // Get the TRS components from the first node and use them as the test data
    // for decomposing.
    const auto *pDefaultTRS = std::get_if<fastgltf::Node::TRS>(
        &assetWithMatrix->nodes.front().transform);
    ...
    const auto *pMatrix = std::get_if<fastgltf::Node::TransformMatrix>(
        &assetWithMatrix->nodes.back().transform);
    ...
    const auto *pDecomposedTRS = std::get_if<fastgltf::Node::TRS>(
        &assetDecomposed->nodes.back().transform);
  ...
  SECTION("Test decomposition against glm decomposition") {
    ...
    std::array<float, 16> matrix = {-0.4234085381031037F,
                                    -0.9059388637542724F,
    ...
    glm::decompose(glmMatrix, glmScale, glmRotation, glmTranslation, glmSkew,
                   glmPerspective);

    // I use glm::epsilon<float>() * 10 here because some matrices I tested this
    // with resulted in an error margin greater than the normal epsilon value. I
    // will investigate this in the future, but I suspect using double in the
    // decompose functions should help mitigate most of it.
    REQUIRE(glm::make_vec3(translation.data()) == glmTranslation);
    REQUIRE(
        glm::all(glm::epsilonEqual(glm::make_quat(rotation.data()), glmRotation,
                                   glm::epsilon<float>() * 10)));
    REQUIRE(glm::all(glm::epsilonEqual(glm::make_vec3(scale.data()), glmScale,
                                       glm::epsilon<float>())));
  }
}

TEST_CASE("Validate sparse accessor parsing", "[gltf-loader]") {
  auto simpleSparseAccessor =
      sampleModels / "2.0" / "SimpleSparseAccessor" / "glTF";
  auto jsonData = std::make_unique<fastgltf::GltfDataBuffer>();
  REQUIRE(jsonData->loadFromFile(simpleSparseAccessor /
                                 "SimpleSparseAccessor.gltf"));

  fastgltf::Parser parser;
  auto asset = parser.loadGLTF(jsonData.get(), simpleSparseAccessor, noOptions,
...
TEST_CASE("Validate morph target parsing", "[gltf-loader]") {
  ...
  fastgltf::Parser parser;
  auto asset = parser.loadGLTF(jsonData.get(), simpleMorph, noOptions,
...
TEST_CASE("Test accessors min/max", "[gltf-loader]") {
  ...
  auto asset = parser.loadGLTF(&jsonData, lightsLamp, noOptions,
                               fastgltf::Category::Accessors);
  REQUIRE(asset.error() == fastgltf::Error::None);
  REQUIRE(fastgltf::validate(asset.get()) == fastgltf::Error::None);

  REQUIRE(std::find_if(asset->extensionsUsed.begin(),
                       asset->extensionsUsed.end(), [](auto &string) {
                         return string ==
...

  {
    ...
    const auto *max = std::get_if<FASTGLTF_STD_PMR_NS::vector<std::int64_t>>(
        &firstAccessor.max);
    const auto *min = std::get_if<FASTGLTF_STD_PMR_NS::vector<std::int64_t>>(
        &firstAccessor.min);
    REQUIRE(max != nullptr);
    REQUIRE(min != nullptr);
...

  {
    ...
    const auto *max =
        std::get_if<FASTGLTF_STD_PMR_NS::vector<double>>(&secondAccessor.max);
    const auto *min =
        std::get_if<FASTGLTF_STD_PMR_NS::vector<double>>(&secondAccessor.min);
    REQUIRE(max != nullptr);
    REQUIRE(min != nullptr);
    ...
    REQUIRE(glm::epsilonEqual(max->at(0), 0.81497824192047119,
                              glm::epsilon<double>()));
    REQUIRE(glm::epsilonEqual(max->at(1), 1.8746249675750732,
                              glm::epsilon<double>()));
    REQUIRE(glm::epsilonEqual(max->at(2), 0.32295516133308411,
                              glm::epsilon<double>()));

    REQUIRE(glm::epsilonEqual(min->at(0), -0.12269512563943863,
                              glm::epsilon<double>()));
    REQUIRE(glm::epsilonEqual(min->at(1), 0.013025385327637196,
                              glm::epsilon<double>()));
    REQUIRE(glm::epsilonEqual(min->at(2), -0.32393229007720947,
                              glm::epsilon<double>()));
  }

  {
    ...
    const auto *max =
        std::get_if<FASTGLTF_STD_PMR_NS::vector<double>>(&fifthAccessor.max);
    const auto *min =
        std::get_if<FASTGLTF_STD_PMR_NS::vector<double>>(&fifthAccessor.min);
    REQUIRE(max != nullptr);
    REQUIRE(min != nullptr);
...
TEST_CASE("Test unicode characters", "[gltf-loader]") {
  auto lightsLamp = sampleModels / "2.0" /
                    std::filesystem::u8path(u8"Unicode❤♻Test") / "glTF";
  fastgltf::GltfDataBuffer jsonData;
  REQUIRE(jsonData.loadFromFile(
      lightsLamp / std::filesystem::u8path(u8"Unicode❤♻Test.gltf")));
