...
TEST_CASE("Test basic URIs", "[uri-tests]") {
  ...
  SECTION("File scheme localhost path") {
    const std::string_view localhostPath =
        "file://localhost/path/somewhere.xyz";
...
TEST_CASE("Test generic URIs", "[uri-tests]") {
  ...
  const fastgltf::URI uri(
      std::string_view("https://john.doe@www.example.com:123/forum/questions/"
  ...
  const fastgltf::URI uri1(
      std::string_view("ldap://[2001:db8::7]/c=GB?objectClass?one"));
...
TEST_CASE("Test data URI parsing", "[uri-tests]") {
  // This example base64 data is from an example on
...
TEST_CASE("Validate URI copying/moving", "[uri-tests]") {
  ...
  SECTION("Move semantics") {
    ...
    // Test that the values were copied over and that the string views are still
    // valid.
...
TEST_CASE("Validate escaped/percent-encoded URI", "[uri-tests]") {
  const std::string_view gltfString =
      R"({"images": [{"uri": "grande_sph\u00E8re.png"}]})";
  fastgltf::GltfDataBuffer dataBuffer;
  dataBuffer.copyBytes((uint8_t *)gltfString.data(), gltfString.size());

  fastgltf::Parser parser;
  auto asset = parser.loadGLTF(&dataBuffer, "",
                               fastgltf::Options::DontRequireValidAssetMember);
  REQUIRE(asset.error() == fastgltf::Error::None);

  auto escaped = std::get<fastgltf::sources::URI>(asset->images.front().data);

  // This only tests wether the default ctor of fastgltf::URI can handle
  // percent-encoding correctly.
