...
static void _mixIOError(const char *function, ULONG ulRC)
{
    debug_os2("%s() - failed, rc = 0x%lX (%s)",
              function, ulRC,
              (ulRC == MCIERR_INVALID_MODE) ? "Mixer mode does not match request" : (ulRC == MCIERR_INVALID_BUFFER) ? "Caller sent an invalid buffer"
                                                                                                                    : "unknown");
}
...
static int OS2_OpenDevice(_THIS, const char *devname)
{
    ...
    ulRC = mciSendCommand(0, MCI_OPEN,
                          (_getEnvULong("SDL_AUDIO_SHARE", 1, 0) != 0) ? MCI_WAIT | MCI_OPEN_TYPE_ID | MCI_OPEN_SHAREABLE : MCI_WAIT | MCI_OPEN_TYPE_ID,
                          &stMCIAmpOpen, 0);
    if (LOUSHORT(ulRC) != MCIERR_SUCCESS) {
        DosCloseEventSem(pAData->hevBuf);
    ...
    if (iscapture) {
        ...
        stMCIConnector.ulConnectorType = (fLineIn) ? MCI_LINE_IN_CONNECTOR : MCI_MICROPHONE_CONNECTOR;
        ...
        stMCIAmpSet.ulValue = (fLineIn) ? MCI_LINE_IN_CONNECTOR : MCI_MICROPHONE_CONNECTOR;
