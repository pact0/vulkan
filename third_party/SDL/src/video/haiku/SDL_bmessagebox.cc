...
class HAIKU_SDL_MessageBox : public BAlert
{
    ...
    ParseSdlMessageBoxData(const SDL_MessageBoxData *aMessageBoxData)
    {
        ...
        (aMessageBoxData->title[0]) ? SetTitle(aMessageBoxData->title) : SetTitle(HAIKU_SDL_DefTitle);
        (aMessageBoxData->message[0]) ? SetMessageText(aMessageBoxData->message) : SetMessageText(HAIKU_SDL_DefMessage);
  ...
  public:
    explicit HAIKU_SDL_MessageBox(const SDL_MessageBoxData *aMessageBoxData)
    ...
    virtual ~HAIKU_SDL_MessageBox(void)
    {
        fButtons.clear();
    }
