...
#define pushLeft                        \
    {                                   \
        stack[stacktop].first = ffirst; \
        stack[stacktop++].last = last;  \
    }
#define pushRight                       \
    {                                   \
        stack[stacktop].first = first;  \
        stack[stacktop++].last = llast; \
    }
#define doLeft          \
    {                   \
        first = ffirst; \
...
#define Pivot(swapper, sz)                              \
    if ((size_t)(last - first) > PIVOT_THRESHOLD * sz)  \
        mid = pivot_big(first, mid, last, sz, compare); \
    else {                                              \
                       ...
                       int compare(const void *, const void *))
{
...
#endif
        m1 = compare(a, b) < 0 ? (compare(b, c) < 0 ? b : (compare(a, c) < 0 ? c : a))
    ...
    }
    {
        ...
        fprintf(stderr, ". %d %d %d @ %p %p %p\n", *(int *)a, *(int *)b, *(int *)c, a, b, c);
#endif
        m2 = compare(a, b) < 0 ? (compare(b, c) < 0 ? b : (compare(a, c) < 0 ? c : a))
                               : (compare(a, c) < 0 ? a : (compare(b, c) < 0 ? c : b));
    }
    {
        ...
        fprintf(stderr, "> %d %d %d @ %p %p %p\n", *(int *)a, *(int *)b, *(int *)c, a, b, c);
#endif
        m3 = compare(a, b) < 0 ? (compare(b, c) < 0 ? b : (compare(a, c) < 0 ? c : a))
                               : (compare(a, c) < 0 ? a : (compare(b, c) < 0 ? c : b));
    }
#ifdef DEBUG_QSORT
    fprintf(stderr, "-> %d %d %d @ %p %p %p\n", *(int *)m1, *(int *)m2, *(int *)m3, m1, m2, m3);
#endif
    return compare(m1, m2) < 0 ? (compare(m2, m3) < 0 ? m2 : (compare(m1, m3) < 0 ? m3 : m1))
